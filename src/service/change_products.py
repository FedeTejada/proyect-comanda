...
def frame_modif_product(frame):
    ...
    def refresh_products():
        ...
        for product in products:
            ...
            CTkLabel(row_frame, text=str(product["id"]), fg_color='#666', text_color='white').grid(row=0, column=0, padx=(10, 20), sticky='w')
            CTkLabel(row_frame, text=product["name"], fg_color='#666', text_color='white').grid(row=0, column=1, padx=(0, 20))
    ...
    def remove_product():
        """
        Removes the last product from the product list and refreshes the display.
        """
        if products:
            products.pop()
            refresh_products()
